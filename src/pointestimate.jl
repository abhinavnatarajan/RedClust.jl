    ...
    if method == "MPEL" # minimum posterior expectated loss
        ...
        if typeof(loss) == Function
            ...
            if loss ∉ ["binder", "omARI", "VI", "NVI", "ID", "NID"]
                ...
                n = length(samples.clusts[1])
                lossfn = @match loss begin
                    "binder" => (x::ClustLabelVector, y::ClustLabelVector) -> binderloss(x, y; normalised = false)
        ...
        @inbounds lossM = [i < j ? lossfn(clusts[i], clusts[j]) : 0 for i in eachindex(clusts), j in eachindex(clusts)]
        lossM += transpose(lossM)
        i = argmin(vec(sum(lossM, dims = 1)))
        return (clusts[i], i)
    end
    if method == "SALSO"
...
Computes the Binder loss between two clusterings. If normalised = true, the result is equal to one minus the rand index between a and b. 
"""
function binderloss(
    a::ClustLabelVector,
    b::ClustLabelVector;
