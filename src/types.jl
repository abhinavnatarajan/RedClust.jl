...
Base.@kwdef mutable struct MCMCResult
    ...
    function MCMCResult(data::MCMCData, options::MCMCOptionsList, params::PriorHyperparamsList)
        ...
        numMH = options.numMH
        x.clusts = [zeros(ClustLabelType, numpts) for _ in 1:numsamples]
...
function Base.show(io::IO, ::MIME"text/plain", result::MCMCResult)
    ...
    println(io, "Using repulsion: $(result.params.repulsion)")
    println(io, "Maximum number of clusters allowed: $((result.params.maxK > 0) ? result.params.maxK : "any")")
